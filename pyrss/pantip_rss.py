...
def generate_entries():
    ...
    links = ['https://pantip.com' + i.get('href') for i in soup.select('div.post-item-title > a')]
    descriptions = titles
    guids = links

    pubdates = [i.get('data-utime') for i in soup.select('div.post-item-by > span.timestamp  > abbr.timeago')]
    pubdates = [parse(i) for i in pubdates]
   #  pubdates = [pytz.timezone('Asia/Bangkok').localize(i) for i in pubdates]

    entries = []
    for title, link, description, guid, pubdate in zip(titles, links, descriptions, guids, pubdates):
        entries.append(
            PyRSS2Gen.RSSItem(
              title = title,
              link = link,
